...
use sql_project;
-- SHEET 1
create table customer_income_crietria select*,
if(applicantincome>15000,'GRADE A',
if(applicantincome>9000,'GRADE B',
if(applicantincome>5000,'MIDDLE CLASS CUSTOMER','LOW CLASS'))) as income_grade,

if(applicantincome<5000 and property_area='rural',3,
if(applicantincome<5000 and property_area='semi rural',3.5,
if(applicantincome<5000 and property_area='urban',5,
if(applicantincome<5000 and property_area='semi urban',2.5,7)))) as monthly_interest

from customer_income;
select * from customer_income_criteria;
alter table customer_income_criteria rename column `customer id` to customer_id;
select * from loan_cibil_score_status_details;
create table customer_interest_analysis select c.*, s.loanamount,s.loan_amount_term,s.cibil_score,s.cibil_score_status,(c.monthly_interest * loanamount)*100 as monthly_interest_amt, (c.monthly_interest * loanamount)*12 as annual_interest_amt  from customer_income_criteria c inner join loan_cibil_score_status_details s on c.customer_id=s.customer_id;
select * from customer_interest_analysis;

-- SHEET 2
select * from loan_status;
set sql_safe_updates=0; 
alter table loan_status drop column `remarks criteria`;
alter table loan_status rename column `cibil score` to cibil_score;
describe loan_status;
alter table loan_status modify loanamount int;
delimiter ??
create trigger rowlevel_trigger before insert on loan_status for each row
begin

if new.loanamount is null then set loanamount= 'loan still proceesing';
end if ;
end ??
delimiter ;
...
loan_id varchar(55),
loanamount varchar(55),
cibil_score int,
cibil_score_status varchar(65),
primary key(loan_id)
);
delete from cibil_score_status_details where loanamount = 'loan still processing'or cibil_score_status='reject customer';
select * from cibil_score_status_details;
select * from loan_status;
select * from loan_cibil_score_status_details;
set sql_safe_updates=0;
create table loan_cibil_score_status_details select c.*,l.cibil_score_status from loan_status c inner join cibil_score_status_details l on c.loan_id=l.loan_id;
delimiter //
create trigger cibil_score_update after insert on loan_status 
for each row
begin
if new.cibil_score>900 then
insert into cibil_score_status_details(loan_id,loanamount,cibil_score,cibil_score_status) values
(new.loan_id,new.loanamount,new.cibil_score, 'high cibil score');

elseif new.cibil_score>750 then
insert into cibil_score_status_details(loan_id,loanamount,cibil_score,cibil_score_status) values
(new.loan_id,new.loanamount,new.cibil_score, 'no penalty');

elseif new.cibil_score>0 then
insert into cibil_score_status_details(loan_id,loanamount,cibil_score,cibil_score_status) values
(new.loan_id,new.loanamount,new.cibil_score, 'penalty customer');

elseif new.cibil_score <=0 then
insert into cibil_score_status_details(loan_id,loanamount,cibil_score,cibil_score_status) values
(new.loan_id,new.loanamount,new.cibil_score, 'reject customer');
...
alter table customer_income rename column `customer id` to customer_id;

create table total_count select s.*,c.customer_name,c.region_id,c.postal_code,c.segment,c.state ,l.loanamount,l.loan_amount_term,l.cibil_score,r.region,d.gender,d.age,d.married,d.education,d.self_employed 
from customer_income s inner join country_state c on s.customer_id=c.customer_id 
inner join loan_status l on c.customer_id=l.customer_id 
inner join region_info r on r.region_id=c.region_id
inner join customer_det d on c.customer_id=d.customer_id;
select count(*) from total_count;

select c.*,r.region from country_state c right join region_info r on c.region_id=r.region_id where c.customer_id is null;
select * from country_state where segment in('home office','corporate');
delimiter //
create procedure all_data()
begin
select s.*,c.customer_name,c.region_id,c.postal_code,c.segment,c.state ,l.loanamount,l.loan_amount_term,l.cibil_score,r.region,d.gender,d.age,d.married,d.education,d.self_employed 
from customer_income s inner join country_state c on s.customer_id=c.customer_id 
inner join loan_status l on c.customer_id=l.customer_id 
inner join region_info r on r.region_id=c.region_id
inner join customer_det d on c.customer_id=d.customer_id;
